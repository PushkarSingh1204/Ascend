  ...
  return (
    <div className="min-h-screen bg-[#07070b] text-foreground flex flex-col items-center justify-center p-4 relative">
      ...
      <div className="flex items-center gap-2 mb-8 cursor-pointer z-10" onClick={() => navigate('/')}>
        <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-blue-500 to-indigo-600 flex items-center justify-center font-bold text-foreground shadow-lg shadow-blue-500/20">
          A
        </div>
      ...
      {/* Authentication Card */}
      <div className="w-full max-w-md glassmorphism p-8 rounded-2xl shadow-xl z-10 border border-border">
        
        {/* Toggle tabs */}
        <div className="flex bg-background/80 p-1.5 rounded-xl border border-border mb-8">
          <button
            onClick={() => { setIsSignUp(false); setError(''); }}
            className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 ${!isSignUp ? 'bg-card text-foreground shadow-md' : 'text-muted-foreground hover:text-foreground'}`}
          >
            Sign In
          </button>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-background/70 border border-border rounded-xl py-3 pl-11 pr-4 text-sm text-foreground focus:outline-none focus:border-blue-500 transition-colors placeholder-neutral-600"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 mt-2 rounded-xl font-bold text-sm text-foreground bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 transition-all duration-300 shadow-md shadow-blue-600/10 flex items-center justify-center gap-2"
          >
            {loading ? (
        ...
        <div className="relative my-8 text-center">
          ...
          <span className="relative bg-[#0d0d12] px-3 text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Or Continue With</span>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 rounded-xl border border-border hover:border-neutral-700 bg-card/45 hover:bg-card/80 text-sm font-semibold text-foreground transition-all duration-300 flex items-center justify-center gap-2"
        >
