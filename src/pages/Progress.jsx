...
function ProgressContent() {
  ...
  const filteredTimelineItems = (timelineItems || []).filter(item => {
    ...
    const matchDate = !filterDate || (item.date && String(item.date).includes(filterDate));
    const matchSearch = !searchQuery || 
      ((item.title || '').toLowerCase().includes(searchQuery.toLowerCase())) ||
      (item.notes && String(item.notes).toLowerCase().includes(searchQuery.toLowerCase())) ||
      (item.reflections && String(item.reflections).toLowerCase().includes(searchQuery.toLowerCase())) ||
      (item.badgeName && String(item.badgeName).toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const loadProgressData = async () => {
    try {
      ...
      const photoItems = safeList.map(p => ({
        ...
        type: 'photo',
        title: `Week ${p.week_number || 1} Photo`,
        notes: p.notes || '',
      ...
      const badgeItems = safeUnlocked.map((badgeId) => {
        ...
        const badge = Array.isArray(BADGES) ? BADGES.find(b => b?.id === badgeId) : null;
        ...
        if (badgeId === 'journal_entry' && safeJournals.length > 0) {
          bDate = safeJournals[safeJournals.length - 1]?.date || joinDate;
        ...
        return {
          ...
          notes: safeBadge.description,
          badgeName: safeBadge.name,
          badgeIcon: safeBadge.icon || '🏆',
          badgeXp: safeBadge.xp || 100,
          badge: safeBadge
        };
      }).filter(Boolean);
      ...
      console.error("[ProgressSlider] Load progress data error:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUploadSubmit = async (e) => {
    ...
    try {
      ...
      await addProgressPhoto(uploadedMetadata, notes);
      ...
      console.error("[ProgressSlider] Upload error:", err);
  ...
  const handleDeletePhoto = async (photoId, publicId) => {
    ...
    try {
      ...
      console.error("[ProgressSlider] Delete photo error:", err);
  ...
  const beforePhoto = photos?.[beforePhotoIdx] || photos?.[0] || null;
  const afterPhoto = photos?.[afterPhotoIdx] || photos?.[photos.length - 1] || null;
        ...
        {photos.length >= 2 ? (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-stretch">
            ...
            <Card className="p-5 flex flex-col justify-between space-y-4 lg:col-span-1">
              <div className="space-y-3">
                <div className="space-y-1">
                  ...
                  >
                    {photos.map((p, idx) => (
                      <option key={p.id || idx} value={idx}>Week {p.week_number || 1} ({p.date || 'Baseline'})</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  >
                    {photos.map((p, idx) => (
                      <option key={p.id || idx} value={idx}>Week {p.week_number || 1} ({p.date || 'Current'})</option>
                    ))}
                  </select>
                </div>
              </div>
            ...
            <div className="lg:col-span-3">
              <ImageSlider 
                beforeImage={beforePhoto ? getOptimizedUrl(beforePhoto.photo_url) : ''} 
                afterImage={afterPhoto ? getOptimizedUrl(afterPhoto.photo_url) : ''}
                beforeLabel={beforePhoto ? `Week ${beforePhoto.week_number || 1}` : 'Week 1'}
                afterLabel={afterPhoto ? `Week ${afterPhoto.week_number || 1}` : 'Current'}
              />
            </div>
          </div>
        ) : (
                ...
                return (
                  <div key={item.id} className="relative group">
                    <span className="absolute -left-10 top-1.5 w-8 h-8 rounded-xl bg-secondary/40 border border-border flex items-center justify-center text-xs group-hover:border-primary/30 transition-colors z-10">
                      {item.type === 'photo' ? '📸' : item.type === 'journal' ? '📝' : (item.badgeIcon || item.badge?.icon || '🏆')}
                    </span>
                    ...
                    <div className="bg-secondary/15 p-5 rounded-2xl border border-border shadow-md space-y-4 hover:bg-secondary/20 transition-all animate-fade-in">
                      ...
                      <div className="flex justify-between items-center text-xs">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-foreground text-sm">{item.title || 'Timeline Event'}</span>
                      ...
                      {item.type === 'photo' && (
                        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                          {item.photoUrl ? (
                            ...
                            <div className="w-24 h-24 rounded-xl bg-secondary border border-border flex items-center justify-center text-muted-foreground text-xs font-bold">
                              No Image
                            </div>
                          )}
                        ...
                        <div className="space-y-3 text-xs">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-1">
                              ...
                              <p className="text-foreground bg-secondary/40 border border-border p-3 rounded-xl leading-relaxed">{item.notes || 'No notes written.'}</p>
                            </div>
                            {item.reflections && (
                              <div className="space-y-1">
                      ...
                      {item.type === 'badge' && (
                        <div className="flex items-center gap-4">
                          <span className="text-3xl bg-secondary/50 p-2.5 rounded-2xl border border-border">
                            {item.badgeIcon || item.badge?.icon || '🏆'}
                          </span>
                          <div>
                            <p className="text-xs font-black text-foreground">
                              {item.badgeName || item.badge?.name || 'Milestone Badge'}: "{item.notes || item.badge?.description || 'Achievement unlocked'}"
                            </p>
                            <span className="text-[8px] font-black text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-0.5 rounded-full mt-1.5 inline-block uppercase tracking-wider">
                              +{item.badgeXp || item.badge?.xp || 100} XP Bonus Awarded
                            </span>
                          </div>
                        </div>
                      )}
            ...
            >
              ...
              <form onSubmit={handleUploadSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest block">Photo File</label>
                  <input 
                    type="file"
                    accept="image/jpeg,image/png,image/webp"
                    onChange={handleFileSelect}
                    disabled={uploadingState}
                    className="block w-full text-xs text-muted-foreground file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-primary file:text-white hover:file:bg-primary/80 cursor-pointer"
                  />
                </div>

                {/* Preview Image */}
                {previewUrl && (
                  <div className="relative rounded-xl overflow-hidden border border-border h-48 bg-secondary/30 flex items-center justify-center">
                ...
                {/* Optional Notes */}
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest block">Notes / Description (Optional)</label>
                  <textarea
                    placeholder="e.g. Week 4 posture check, cold shower morning rinse..."
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    disabled={uploadingState}
                    className="w-full bg-secondary/40 border border-border rounded-xl p-3 text-xs text-foreground focus:outline-none focus:border-primary placeholder-muted-foreground h-20 resize-none"
                  />
                </div>
                ...
                {uploadingState && (
                  <div className="space-y-2 pt-2">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-foreground">Uploading image...</span>
                    ...
                    <button
                      ...
                      className="text-xs text-destructive hover:underline font-bold block mx-auto mt-2"
                    >
                      Cancel Upload
                    </button>
                  </div>
                ...
                {successAnimation && (
                  <div className="py-4 text-center space-y-2 text-emerald-400">
                    ...
                    <p className="text-xs font-bold">Progress Photo Uploaded (+20 XP)!</p>
                  </div>
                )}

                {/* Action Buttons */}
                {!uploadingState && !successAnimation && (
...
/**
 * Wrapped in an isolated ErrorBoundary to guarantee that any local failure
 * inside ProgressSlider can never bubble up or crash the rest of the application.
 */
